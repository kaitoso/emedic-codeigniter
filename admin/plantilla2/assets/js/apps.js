...
var handleThemePageStructureControl = function() {
    ...
    if ($.cookie && $.cookie('theme')) {
        ...
        var cssFileSrc = '../plantilla2/assets/css/theme/' + $.cookie('theme') + '.css';
    ...
    $('.theme-list [data-theme]').click(function() {
        var cssFileSrc = '../plantilla2/assets/css/theme/' + $(this).attr('data-theme') + '.css';
        $('#theme').attr('href', cssFileSrc);
...
var handleResetLocalStorage = function() {
    ...
    $(document).on('click', '[data-click=reset-local-storage]', function(e) {
        ...
        '                <h4 class="modal-title"><i class="fa fa-refresh m-r-5"></i> Restablecer almacenamiento local</h4>'+
        '            </div>'+
        '            <div class="modal-body">'+
        '                <div class="alert alert-info m-b-0">¿Desea RESETEAR y/o borrar el Almacenamiento local?</div>'+
        '            </div>'+
        '            <div class="modal-footer">'+
        '                <a href="javascript:;" class="btn btn-sm btn-white" data-dismiss="modal"><i class="fa fa-close"></i> No</a>'+
        '                <a href="javascript:;" class="btn btn-sm btn-danger" data-click="confirm-reset-local-storage"><i class="fa fa-check"></i> Sí</a>'+
        '            </div>'+
        '        </div>'+
        '    </div>'+
        '</div>';
...
var handleLoadPage = function(hash) {
    ...
    $.ajax({
        ...
        success: function(data) {
	        $('#tabs_menu [href="' + hash + '"]').parent().css('display','inline-block').addClass('active');
	        $('#tabs_menu [href="' + hash + '"]').parent().siblings('li').removeClass('active');
            $('#content '+hash).html(data).addClass('in active');
            $('#content '+hash).siblings('div').removeClass('in active');
            //console.log(hash);
            $('html, body').animate({
                scrollTop: $("body").offset().top
            }, 250);
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            $('#content '+hash).html(default_content);
        }
    });
};*/
