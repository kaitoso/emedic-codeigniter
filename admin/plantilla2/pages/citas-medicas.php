...
?>
	...
	<div id="content" class="content">
	    ...
	    <!-- begin row -->
		<div class="row">
		    <div class="col-12 col-md-4 col-sm-4" id="sandbox-container">
		        <div><!-- class="menu_fixed"-->
		            <form>
		                <div class="form-group">
		                    <label for="medicoSelect"><strong>Médico</strong></label>
		                    ...
		                    <select class="form-control" id="prestacionSelect" style="">
		                        <option value="3">Consulta Dental</option>
		                        <option value="2">Consulta Pediatrica</option>
		                        <option value="6">Consulta Testing</option>
		                    </select>
		                </div>
		                <hr>
		                <div class="form-group row" id="pacienteDIV" style="">
		                    <div class="col-sm-12"><label for="Paciente"><strong>Paciente</strong></label></div>
		                    <div class="col-sm-12">
		                        <div class="btn-group bootstrap-select form-control">
		                            <select class="form-control selectpicker" id="pacienteSelect" onchange="select_paciente(this.value);" data-live-search="true" tabindex="-98">
		                                <option>-- Seleccione paciente --</option>
		                                <option value="9" selected>Pedro Fernandez Palominos</option>
		                                <option value="11">Test Demo2 Demo</option>
		                                <option value="12">test paterno materno</option>
		                                <option value="15">Mario Carrillo Rivera</option>
		                        ...
		                        <br>
		                        <label id="rut_paciente" style=""><b>RUT: </b>35772hh</label><br>
		                        <div class="alert alert-danger" id="cuadro-alertas" style="">
		                            <a id="alertas_paciente" style="" title="" data-trigger="hover" data-html="true" data-toggle="alertas" data-placement="top" data-content="<ul><li>24-11-2017-: Cancela para pedro</li><li>30-11-2017-: test</li><li>30-11-2017-: sin motivo</li></ul>" data-original-title="Alertas">Este paciente tiene 3 alertas</a>
		                        </div>
		                        <input class="form-control" id="fecha_seleccionada" value="19-01-2018" style="display:none;" type="text"> 
		                        <br>
		                        <a href="#" id="link_editar_paciente" class="btn btn-primary" data-toggle="modal" data-target="#modalEditarPaciente" style="">Editar Paciente</a>
		                        <a href="#" id="link_sobrecupo" class="pull-right btn btn-warning" data-toggle="modal" data-target="#modalSobreCupo">AGENDAR SOBRECUPO</a>
		                        <br>
		                    </div>
		                </div>
		            </form>
		            <hr>
		            <div class="alert alert-primary alert_busca" style="display: none;" role="alert">
							Horario del médico: <strong id="nombre_medico"></strong>.
					</div>
		            <div class="calendar_horas">
			            	<center>
			            		<div id="myDiv">
									<img id="loading-image2" src="https://icons8.com/preloaders/preloaders/35/Fading%20lines.gif" style="display:none;"/>
							    </div>
			            	</center>
		            </div>
		    ...
		    <div class="col-12 col-md-8 col-sm-8">
		        ...
		        <center>
		            <div id="myDiv">
		                <img id="loading-image-tabla" src="https://icons8.com/preloaders/preloaders/35/Fading%20lines.gif" style="display: none;">
		            </div>
		        </center>
		            ...
		            <thead>
		                <tr>
		                    <td class="bg-primary text-center" colspan="4">
		                        <label id="td_fecha_titulo">------CITAS DE HOY------</label>
		                    </td>
		                </tr>
		                <tr>
		                    <th>Hora</th>
	...
	<script type="text/javascript">
	    $(document).ready(function() {
		    ...
		    $('.filter_status_all').on('click', function(){
		    		$('.bg-danger').removeClass('invisible');
		    		$('.bg-warning').removeClass('invisible');
	            $('.bg-info').removeClass('invisible');
	            $('.bg-white').removeClass('invisible');
		    });
		    $('.filter_status_disponible').on('click', function(){
	        		$('.bg-danger').addClass('invisible');
	        		$('.bg-warning').addClass('invisible');
				$('.bg-info').removeClass('invisible');
				$('.bg-white').addClass('invisible');
		    });
		    $('.filter_status_reservado').on('click', function(){
		    		$('.bg-danger').removeClass('invisible');
	            $('.bg-info').addClass('invisible');
	            $('.bg-warning').addClass('invisible');
	            $('.bg-white').addClass('invisible');
		    });
		    $('.filter_status_fuera_horario').on('click', function(){
			    $('.bg-info').addClass('invisible');
			    $('.bg-danger').addClass('invisible');
			    $('.bg-warning').removeClass('invisible');
			    $('.bg-white').removeClass('invisible');
		    });
		    $('select#medicoSelect').on('change',function(){
		    	var el = document.getElementById('medicoSelect');
				var text = el.options[el.selectedIndex].innerHTML;
			    if(text != '-- Seleccione Médico --' ){
				    $( "#nombre_medico" ).empty();
					$( "#nombre_medico" ).append(text);
					$( ".alert_busca" ).css('display', 'block');
			    }else{
				    $( "#nombre_medico" ).empty();
				    $( ".alert_busca" ).css('display', 'none');
			    }
			});
		});
